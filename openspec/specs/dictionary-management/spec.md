...
Gives an admin user CRUD control over the controlled-vocabulary dictionaries (colors, materials, hardware items, standard sizes) and per-part-role size rules that the product configuration and catalog matching capabilities depend on, so no attribute value is ever entered as free text.
