...
Lets a manager select parts (top/bottom/facade/corpus/hardware) for a single product type (nightstand-like) from controlled dictionaries, validates part sizes against dictionary/rule constraints, and generates a bill of materials (BOM) only when the selection is valid.
